const GIST_ID  = process.env.GIST_SERVICES_ID;
...
async function writeGist(data) {
  await fetch(`https://api.github.com/gists/${GIST_ID}`, {
    ...
    body: JSON.stringify({ files: { 'kevfix-services.json': { content: JSON.stringify(data) } } }),
  });
}
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (!Array.isArray(services)) return res.status(400).json({ error: 'Invalid' });
    await writeGist({ services });
